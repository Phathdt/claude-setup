  ...
  function main() {
    try {
      ...
      if (counts) {
        ...
        lines.push(
          `Progress: ${counts.completed}/${counts.total} done. ${counts.pending} pending, ${counts.inProgress} in progress.`
        );
        if (remaining === 0) {
          lines.push('');
          lines.push(
            '**All tasks completed.** Consider shutting down teammates and synthesizing results.'
          );
        }
      }
