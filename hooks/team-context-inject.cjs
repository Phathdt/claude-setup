...
try {
  ...
  /**
   * Build kit stack context from environment variables
  ...
  function main() {
    try {
      ...
      if (tasks) {
        lines.push(
          `Task summary: ${tasks.pending} pending, ${tasks.inProgress} in progress, ${tasks.completed} completed`
        );
      }
      ...
      if (ckCtx.length > 1) {
        // >1 = has env vars beyond the always-present commit convention line
        lines.push('');
        lines.push('## Kit Context');
        lines.push(...ckCtx);
      }

      lines.push('');
      lines.push(
        'Remember: Check TaskList, claim tasks, respect file ownership, use SendMessage to communicate.'
      );
