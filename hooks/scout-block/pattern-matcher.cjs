 ...
 *
 * Uses 'ignore' package for .kitignore parsing and path matching.
...
// Default patterns if .kitignore doesn't exist or is empty
...
/**
 * Load patterns from .kitignore file
 * Falls back to DEFAULT_PATTERNS if file doesn't exist or is empty
 *
 * @param {string} ckignorePath - Path to .kitignore file
...
function loadPatterns(ckignorePath) {
  ...
  try {
    ...
    console.error('WARN: Failed to read .kitignore:', error.message);
    return DEFAULT_PATTERNS;
  }
}
 ...
 *
 * @param {string[]} patterns - Array of patterns from .kitignore
 ...
 *
 * @param {string[]} originalPatterns - Original patterns from .kitignore
