    ...
    try {
      ...
      if (taskInfo) {
        ...
        if (taskInfo.unblocked.length > 0) {
          lines.push(
            `Unblocked & unassigned: ${taskInfo.unblocked.map((t) => `#${t.id} "${t.subject}"`).join(', ')}`
          );
          ...
          lines.push(
            `All remaining tasks are blocked or assigned. ${teammate_name} may be waiting for dependencies.`
          );
        }
      }
