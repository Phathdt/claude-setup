/**
 * Shared utilities for Claude Code Kit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.kit.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.kit.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `kit-session-${sessionId}.json`);
}
 ...
 * @param {Object} config - Claude Code Kit config
 ...
 *   2. Global config (~/.claude/.kit.json) - user preferences
 *   3. Local config (./.claude/.kit.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.CK_DEBUG) {
      console.error(`[kit-config] Warning: ${validation.error}`);
    }
  }
