...
/**
 * Load .kit.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .kit.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.kit.json')
      : path.join(process.cwd(), '.claude', '.kit.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .kit.json config
