...
const USAGE_CACHE_FILE = path.join(os.tmpdir(), 'kit-usage-limits-cache.json');
...
function buildContextSection(sessionId) {
  ...
  try {
    const contextPath = path.join(os.tmpdir(), `kit-context-${sessionId}.json`);
 ...
 * @param {Object} [params.config] - Kit config (auto-loaded if not provided)
