 ...
 * @param {string} [params.options.kitignorePath] - Path to .kitignore file
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Resolve .kitignore path
  const resolvedCkignorePath = ckignorePath || path.join(claudeDir, '.kitignore');
