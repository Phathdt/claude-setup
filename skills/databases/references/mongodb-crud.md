...
```javascript
// $set (set field value)
db.users.updateOne({ _id: userId }, { $set: { status: 'active', updatedAt: new Date() } });
...
db.posts.updateOne({ _id: postId }, { $push: { tags: { $each: ['mongodb', 'database'] } } });
...
db.posts.updateOne({ _id: postId }, { $set: { 'comments.$[].verified': true } });
...
db.users.findOneAndUpdate({ email: 'alice@example.com' }, { $set: { status: 'active' } });
