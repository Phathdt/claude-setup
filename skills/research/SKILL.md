...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `$HOME/.claude/.kit.json` (or `~/.claude/.kit.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `$HOME/.claude/.kit.json`: `gemini.model` (default: `gemini-3-flash-preview`)
